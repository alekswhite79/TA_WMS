    ...
    if len(records) > 0:
        ...
        for record in records:
            if record['СпланированноеКоличество'] == record['ФактическоеКоличество']: # не добавлять карточку если позиция собрана
                continue
            if record['Исполнитель'].strip() != "" and record['Исполнитель'].strip() != hashMap.get('user'): # не добавлять карточку если позиция собирается другим исполнителем
                hashMap.put("toast", str(len(record['Исполнитель'])))
                continue
            c = {
                "key": record['Код'],
                ...
                "Исполнитель": record['Исполнитель'],
...
def Update_Qty_Goods(hashMap, card_of_goods):  # , qty=1):
    ...
    #                                              {"НомерЗаказа": hashMap.get('НомерЗаказа')}]})
    # if not CurrentOrder[0]['ЗаказВРаботе']:
    #     db["OrdersForSelection"].update({"$and": [{"ВидЗаказа": hashMap.get('ВидЗаказа')},
    #                                              {"НомерЗаказа": hashMap.get('НомерЗаказа')}]},
    #                                     {})        
    ...
    hashMap.put("ПозицияДляОбновления", json.dumps(card_of_goods))
...
def py_onInput_auth(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == 'pin':  # введен пин
        ...
        if hashMap.get("pin") == "1111" or (len(result) == 1 and hashMap.get("pin") == result[0]['PIN']):
            # if hashMap.containsKey("TestVar"):
