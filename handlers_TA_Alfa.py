...
def py_LoadGoods(hashMap):
    ...
    db = pelicans["TA_WMS"]
    records = db["GoodsForSelection"].find({"НомерЗаказа":hashMap.get("НомерЗаказа"),"ВидЗаказа":hashMap.get("ВидЗаказа")})
        # {"$and":[
    ...
    if len(records)>0:
        ...
        for record in records:
            c =  {
                ...
                "КоличествоСпланировано": record['КОтбору'],
                "КоличествоОтобрано": record['Отобрано'],
                "Код": record['Код'],
